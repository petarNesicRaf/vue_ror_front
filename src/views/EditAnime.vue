<template>
    <div class="d-flex justify-content-center align-items-center" style="margin-top: 100px;">
        ...
        <form @submit.prevent="submitForm" class="p-4 bg-light rounded shadow-sm" > <!-- Adjust the max-width as needed -->
            ...
            <div class="mb-3">
                <label for="newDirectorFirstName" class="form-label">Chapters</label>
                <input type="text" class="form-control" id="newDirectorFirstName" v-model="chapters">
                <div class="text-danger">{{ formErrors.chapters }}</div>
            </div>
            <div class="mb-3">
            ...
            <div class="mb-3">
                <label for="newDirectorFirstName" class="form-label">Studio</label>
                <input type="text" class="form-control" id="newDirectorFirstName" v-model="studio">
                <div class="text-danger">{{ formErrors.studio }}</div>
            </div>
...
export default {
    ...
    methods: {
        validateForm() {
            ...
            if (!this.chapters || isNaN(this.chapters)) {
                this.formErrors.chapters = "Chapters are required and must be a number";
            }
            if (!this.studio) {
                this.formErrors.studio = "Studio is required";
            }
            if (!this.description) {
        ...
        submitForm() {
            if (this.validateForm()) {
                const createData = {
                    ...
                    chapters: parseInt(this.chapters),
                    studio: this.studio,
                    genre: {
                ...
                this.$axios.put(`/anime/${this.anime_id}`, createData)
                    .then(response => {
