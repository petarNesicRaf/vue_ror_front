...
export default {
    ...
    methods: {
        ...
        submitForm() {
            if (this.validateForm()) {
                ...
                this.$axios.put(`/movies/${this.movie_id}`, updateData)
                    .then(response => {
    ...
    created(){
        this.$axios.get('/genres').then((response) => {
            this.genres = response.data.map((genreData)=>{
                return{
                    genre_id: genreData.id,
                    keyword: genreData.keyword

                }
            });
        });
        this.$axios.get('/director').then((response) => {
            this.directors = response.data.map(directorData=>{
                return{
